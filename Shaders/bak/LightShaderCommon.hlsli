...
float SampleCascadedShadowMap(float4 shadowPosH[3], float NdotL)
{
	...
	else if (CascadedShadowCount > 1)
	{
		...
		{
			shadow = min(shadow, SampleShadowMap(ShadowDepthTexture[0], shadowPosH[0].xyz, shadowOffset[0]));
		}
		if (CascadedShadowCount > 1 &&
		...
		{
			shadow = min(shadow, SampleShadowMap(ShadowDepthTexture[1], shadowPosH[1].xyz, shadowOffset[1]));
		}
		if (CascadedShadowCount > 2 &&
			shadowPosH[2].x > 0 && shadowPosH[2].x < 1 &&
			shadowPosH[2].y > 0 && shadowPosH[2].y < 1 &&
			shadowPosH[2].z > 0 && shadowPosH[2].z < 1)
		{
			shadow = min(shadow, SampleShadowMap(ShadowDepthTexture[2], shadowPosH[2].xyz, shadowOffset[2]));
		}
	}
