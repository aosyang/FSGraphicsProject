...
#include "RenderSystem/D3DCommonPrivate.h"
...
RTexture* RResourcePreviewBuilder::GeneratePreviewTexture(RResourceBase* Resource, int Width, int Height)
{
	if (Resource)
	{
		...
		ComPtr<ID3D11Texture2D>			RenderTargetDepthBuffer;
		ComPtr<ID3D11DepthStencilView>	RenderTargetDepthView;

		// The shader resource view needs to stay with the render system. Don't use ComPtr until further engine code changes.
		...
		GRenderer.D3DDevice()->CreateRenderTargetView(RenderTargetBuffer.Get(), 0, &RenderTargetView);
		...
		GRenderer.D3DDevice()->CreateShaderResourceView(RenderTargetBuffer.Get(), &rtsrvDesc, &RenderTargetSRV);
		...
		GRenderer.D3DDevice()->CreateDepthStencilView(RenderTargetDepthBuffer.Get(), 0, &RenderTargetDepthView);
		...
		GRenderer.SetRenderTargets(1, RenderTargetView.GetAddressOf(), RenderTargetDepthView.Get());
		...
		if (RSceneObject* MeshObject = CreatePreviewSceneObject(PreviewScene, Resource))
		{
		...
		// Now make the preview a texture resource and keep it within the resource manager
