...
namespace ManagedEngineWrapper
{
	...
	public:
		...
		property ManagedShader^ Shader
		{
			ManagedShader^ get()
			{
				ShaderWrapper.SetInstance(material->GetShader());
			...
			{
				ShaderWrapper = value;
				material->SetShader(ShaderWrapper.GetInstance());
			}
		}
...
//		{ \
//			String^ get()			{ return material->GetTextureBySlot(n) ? gcnew String(material->GetTextureBySlot(n)->GetAssetPath().c_str()) : gcnew String(""); } \
//			void set(String^ value)	{ material->GetTextureBySlot(n) = value != "" ? RResourceManager::Instance().FindResource<RTexture>(static_cast<const char*>(Marshal::StringToHGlobalAnsi(value).ToPointer())) : nullptr; } \
//		}
//
...
//		DECLARE_TEXTURE_PROPERTY(7)
		...
		String^ ToString() override
		{
			...
			if (material->GetShader())
				str += gcnew String(material->GetShader()->GetName().c_str());
